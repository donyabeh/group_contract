...
broom::tidy(hap_lex_lm) |>
  kable(align = "cccrr", col.names = c("Term", "Estimate", "Std Error", "Statistic", "P-value"), caption = "Simple Linear Regression Fit")
```
...
table <- augment(hap_lex_lm) |>
  ...
  select(response_var, fitted_var, residual_var) |>
  distinct()
...
# var function https://www.rdocumentation.org/packages/cmvnorm/versions/1.0-7/topics/var
```
